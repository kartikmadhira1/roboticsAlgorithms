...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 29 16:55:04 2019
...
def astar(startX,startY,goalX,goalY,visitMap,ax):
    #heap initialize
    h=[]
    ...
    #(cost,x,y,last node)
    currentList=[]
    heap.heappush(h,(0+heuristic(startX,startY,goalX,goalY),startX,startY,-1,-1,0))
    motion=motionModel()
    map_=visitMap
    currX=[]
    currY=[]
    #map_=build_costmap(pathXpathX,pathY,robot_reach)
    while 1:
...
# Add the grid
        ...
        for i in range(len(motion)):
            if(map_[current[1]+motion[i][0]][current[2]+motion[i][1]]==False):
                ...
                heurWeight=heuristic(current[1]+motion[i][0],current[2]+motion[i][1],goalX,goalY)
                #print(heurWeight)
                gCost=motion[i][2]+current[5]
                heurWeight=(heurWeight)+gCost
                heap.heappush(h,(heurWeight,current[1]+motion[i][0],current[2]+motion[i][1],current[1],current[2],gCost))
...
def heuristic(a,b,goalX,goalY):
    d=math.sqrt((a-goalX)**2+(b-goalY)**2)
    #print(50*d)
    return d
...
def main():
    ...
    if animation:
        ...
        ax.set_xlim([0,300])
        ax.axis("equal")
    path=astar(startX,startY,goalX,goalY,visitMap,ax)
    end=time.time()
