...
class RRT(object):
    def __init__(self,edges=[],nodes=[],path=dict(),step_dist=1,cost_radius=6):
        self.root=None
        self.edges=edges
        self.nodes=nodes
        self.path=path
    ...
    def extend_check(self,nodes,from_x,from_y,obs_list):
        ...
        for a in obs_list:
            if(math.sqrt((nodes.x_cor-from_x)**2+(nodes.y_cor-from_y)**2)>math.sqrt((nodes.x_cor-a[0])**2+(nodes.y_cor-a[1])**2)):
                ...
                #print(abs(dist),a[2])
                if(abs(dist)<a[2]*self.cost_radius*8):
                    return False
        return True
...
def main():
    ...
    while 1:
        ...
        #calculates the heuristic and returns the best node with the lease distance to goal.
        from_list=rrt.heur_calc(samples,goal_x,goal_y)
        print(from_list[1],from_list[2])
        parent_list=rrt.parent_check(from_list[1],from_list[2],obstacleList)
        #print(parent_list)
