...
"""
Basic implementation of Rapidly Exploring Random Trees Star(RRT) with added euclidean distance heuristic
References:
...
class RRT(object):
    def __init__(self,edges=[],nodes=[],path=dict(),step_dist=6,cost_radius=4):
        self.root=None
        self.edges=edges
        self.nodes=nodes
        self.path=path
...
def main():
    ...
    plt.plot(ox, oy, ".k")
    rrt=RRT(step_dist=6,cost_radius=4)
    ...
    while 1:
        ...
        for _ in range(100):
             ...
             if(goal_reach[0]==True):
                 samples.append([goal_reach[1],goal_reach[2],goal_reach[3],goal_reach[4]])
        from_list=rrt.heur_calc(samples,goal_x,goal_y)
        rrt.add_edge(from_list[3],from_list[4],from_list[1],from_list[2],2)
            #print(scaled_x,scaled_y)
        rrt.path[(from_list[1],from_list[2])]=(from_list[3],from_list[4])
        d=math.sqrt((from_list[1]-goal_x)**2+(from_list[2]-goal_y)**2)
        if(d<goal_radius):
                #print(goal_x,goal_y,"******")
            rrt.path[(goal_x,goal_y)]=(from_list[1],from_list[2])
            rrt.add_edge(from_list[1],from_list[2],goal_x,goal_y,2)
            print("Bullseye ;")
            break
